...
describe("modal helpers", () => {
  ...
  test("focusStartPanel scrolls to lobby", () => {
    ...
    focusStartPanel();
    expect(lobby.scrollIntoView).toHaveBeenCalledWith({
      behavior: "smooth",
      block: "nearest",
    });
  });
...
describe("sendChat", () => {
  test("sends trimmed message and clears the input", () => {
    ...
    sendChat();
    expect(globalThis.sendJson).toHaveBeenCalledWith({
      type: "chat",
      text: "Hello world",
    });
...
describe("renderActiveRooms", () => {
  test("shows the section and appends a row per room", () => {
    ...
    expect(document.getElementById("active-rooms-table-wrap").hidden).toBe(
      false,
    );
    expect(document.getElementById("active-rooms-cards-wrap").hidden).toBe(
      false,
    );
    ...
    const cards = document.querySelectorAll(
      "#active-rooms-cards-wrap .active-rooms-card",
    );
...
describe("landing guide carousel", () => {
  ...
  test("carousel next button advances slides", () => {
    ...
    expect(
      document.getElementById("landing-guide-carousel-track").style.transform,
    ).toBe("translateX(-100%)");
  });
});
...
describe("landing quick join nickname", () => {
  ...
  test("quickJoinFromLanding calls quick-join API and stores room code", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("/api/quick-join", {
      method: "POST",
    });
  ...
  test("createRoomAndEnter calls create-room API and redirects", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("/api/rooms", { method: "POST" });
  });
...
describe("room phase helpers", () => {
  ...
  test("renderLobbyRoster shows ready badges and eight slots", () => {
    document.body.innerHTML =
      '<div id="lobby-roster"></div><span id="lobby-player-count"></span>';
    ...
    expect(document.getElementById("lobby-player-count").textContent).toContain(
      "2/8",
    );
  });
  ...
  test("clampNickname limits nick length", () => {
    ...
    expect(clampNickname("abcdefghijklmnopqrstuvwxyz")).toBe(
      "abcdefghijklmnop",
    );
  ...
  test("setRoomPhase lobby shows empty roster slots before score_update", () => {
    ...
    expect(document.getElementById("lobby-player-count").textContent).toBe(
      "0/8",
    );
  });
});
...
describe("tryAutoJoin", () => {
  test("hides inline join, shows lobby shell and connects immediately", () => {
    ...
    expect(document.getElementById("room-inline-join").style.display).toBe(
      "none",
    );
