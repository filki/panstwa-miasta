...
export function getMyNick(): string {
  return myNick;
}
...
export function getWsGeneration(): number {
  return wsGeneration;
}
...
function startRoundTimer(timeLeft: number) {
...
export function connect(roomId?: string, maxRounds?: number, timeLimit?: number, visibility?: string) {
  ...
  socket.onmessage = (event) => {
    ...
    try {
      ...
      const handler = MESSAGE_HANDLERS[msg.type as keyof typeof MESSAGE_HANDLERS];
...
function detectRoomId(): string | undefined {
...
function onLobbyState(msg: any) {
  ...
  roomStore({
    ...
    disconnected_players: disconnected_players || null,
    is_playing: false,
    game_over: false,
    results_phase_active: false,
  });
  ...
  if (rid && pageStore.page !== 'room') {
    pageStore({ page: 'room' });
    history.pushState({}, '', `/room/${rid}`);
  }
  roomStore({ room_id: rid });
}

function onRoundStarted(msg: any) {
  const { letter, time_limit: timeLeft, sender, resume } = msg;
  clearClientRoundTimers();
  roundStore({
    letter: letter || '',
    time_left: timeLeft || 90,
    round_active: true,
    stopped: false,
  ...
  if (!resume) {
    // Fresh round — show countdown then lottery
    runRoundStartCountdown(() => {
      overlayStore({ lottery: true, lotteryLetter: '?' });
      runLetterLottery(letter || '?', () => {
        overlayStore({ lottery: false });
      });
    });
  }

  startRoundTimer(timeLeft || 90);
}
...
function onRoundResults(msg: any) {
  ...
  if (gameOver) {
    roomStore({ is_playing: false, game_over: true, results_phase_active: false });
  }
}

function onScoreUpdate(msg: any) {
  roomStore({ scores: msg.scores });
}

function onChatMessage(msg: any) {
  const messages = chatStore.messages.slice(-99);
  messages.push({ type: 'chat', sender: msg.sender, text: msg.text });
  chatStore({ messages });
}

function onSystemMessage(msg: any) {
  const messages = chatStore.messages.slice(-99);
  messages.push({ type: 'system', text: msg.text });
  chatStore({ messages });
}

function onKicked(_msg: any) {
  isLeaving = true;
  ws?.close();
  leaveRoom();
}

function onKickDenied(_msg: any) {
  // noop
}

function onRoomDissolved(_msg: any) {
  isLeaving = true;
  ws?.close();
  leaveRoom();
}

function onVetoUpdate(msg: any) {
  resultsStore({ veto_tallies: msg.veto_tallies });
}

function onGameRestarted(_msg: any) {
...
function onAppealToken(msg: any) {
  appealStore({ token: msg.token });
}
...
function runRoundStartCountdown(onDone: () => void) {
  ...
  const interval = setInterval(() => {
    ...
    if (step <= 0) {
      ...
      overlayStore({ roundCountdownNum: step });
    }
  }, 700);
}

function runLetterLottery(targetLetter: string, onDone: () => void) {
  ...
  const interval = setInterval(() => {
    ...
    if (progress >= 1) {
      clearInterval(interval);
      overlayStore({ lotteryLetter: targetLetter, lottery: true });
      setTimeout(() => {
        overlayStore({ lottery: false });
        onDone();
      }, 600);
      ...
      overlayStore({ lotteryLetter: alphabet[randomIndex] });
    }
  }, intervalTime);
}
...
export function sendChat(text: string) {
  if (!text.trim()) return;
  sendJson({ type: 'chat', text: text.trim() });
}
...
export function sendAnswers(answers: Record<string, string>) {
  sendJson({ type: 'answers', answers });
}

export function requestRestart(rounds: number, limit: number) {
  sendJson({ type: 'restart_game', rounds, limit });
}
...
export function kickPlayer(name: string) {
  sendJson({ type: 'kick', target: name });
}

// --- API ---

export async function quickJoin(): Promise<string | null> {
  const resp = await fetch('/api/quick-join', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ nick: nickStore.nick }),
  });
...
export async function createRoom(rounds: number, limit: number, visibility: string): Promise<string | null> {
  const resp = await fetch('/api/create-room', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ rounds, limit, visibility, nick: nickStore.nick }),
  });
  if (!resp.ok) return null;
  const data = await resp.json();
  return data.room_id;
