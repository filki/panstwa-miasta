...
function showCreateModal() {
    const landingNick = document
        .getElementById("landing_nickname")
        ?.value.trim();
...
async function createRoomAndEnter() {
    ...
    const timeLimit = Number(
        document.getElementById("time_limit")?.value || 90,
    );
    const visibility =
...
function setRoomPhase(phase) {
    ...
    if (gameMain)
        gameMain.hidden = next === "lobby" || next === "round_results";
...
function buildRoomCard(room) {
    ...
    `;
    li.querySelector(".active-rooms-card-join")?.addEventListener(
        "click",
        () => {
            globalThis.joinRoom(room.id);
        },
...
/* ---------- Room polling dla landingu (powiadomienia o nowych pokojach) ---------- */
let pmLastRoomCount = 0;
...
async function pollActiveRooms() {
    ...
    try {
        ...
        if (count > pmLastRoomCount && pmLastRoomCount > 0) {
            const diff = count - pmLastRoomCount;
            tryNotify(
                "Państwa-Miasta",
                `${diff > 1 ? diff + " nowe" : "Nowy"} ${diff > 1 ? "pokoje" : "pokój"} czeka na graczy!`,
            );
        }
        pmLastRoomCount = count;
    } catch (e) {
...
function startRoomPolling() {
    if (typeof tryNotify !== "function") return;
    pmLastRoomCount = 0;
    // Fetch immediately, then every 60s
    pollActiveRooms();
    setInterval(pollActiveRooms, 60000);
}

// Auto-start on landing page
