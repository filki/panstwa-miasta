...
Czytaj od dołu (max 5 wersji wstecz).

**Numeracja:** semver — kamienie milowe produktu. `pyproject.toml` nadal `0.1.0`.
**Service worker:** `static/sw.js` ma własną linię `pm-vN` — podbijaj przy zmianach JS/CSS, które muszą przebić cache PWA.

---

## [0.1.0] — 2026-05-08

**Proces / intencja:** MVP — Państwa-Miasta online po polsku, WebSocket na pokój, bez frameworka na froncie — szybka iteracja i niski koszt hostingu.
...
**Proces / intencja:** Sesja gry nie może ginąć przy odświeżeniu; host i rozwiązanie pokoju to podstawowy model uprawnień.
...
**Proces / intencja:** Walidacja offline i przewidywalna: mniej Wikidata w runtime, więcej seedów w SQLite. Dane statyczne = tabele + `reload_*` przy starcie.

### Zmiany
- Miasta, państwa, imiona, zawody, zwierzęta, rośliny w seedach / SQL.
- Kolejka liter bez powtórek w kolejnych grach w tym samym pokoju (`_recent_letters` deque, capacity 7).
...
**Proces / intencja:** Solo dev — CI + deploy muszą być powtarzalne; reconnect i limity to warunek gry wieloosobowej na jednym procesie.
...
**Proces / intencja:** Smartphone-first i konkurencja z Gartic Phone: landing i lobby jako pierwsze wrażenie.
...
**Proces / intencja:** Gra ma domykać rundę społecznie (veto na Rzecz) i udostępniać wynik bez konta. Lobby publiczne musi się nie zapychać martwymi pokojami.
...
**Proces / intencja:** Produkcja na Hetznerze — twardzenie przed ruchem: zgadywalne ID pokoi i odwołania bez sesji to realne ryzyka.
...
**Proces / intencja:** Przygotowanie pod wzrost organiczny i lekki dochód bez reklam w rundzie.

### Zmiany
- `robots.txt`, `sitemap.xml`, canonical i OG na landingu, `noindex` na `/room/*` i `/share/*`.
- Stopka: BuyCoffee; hook Umami przez env.
- Landing: CTA wsparcia, iteracje SEO sprint 2.

---

## [0.9.0] — 2026-05-14

**Proces / intencja:** Dwa równoległe tory: Turso jako primary oraz RAG na `dictionary_suggestions`. Hipoteza: embedded replica na VPS + sync do chmury.
...
**Proces / intencja:** Incydenty: rozłączenie w rundzie (timer 0, STOP), lobby 9/8 graczy, za krótkie podsumowanie rundy. Priorytet: spójny stan WS i roster tylko z aktywnych socketów.
...
**Proces / intencja:** Gracze zgłaszają słowa spoza słownika; pełny RAG na produkcji wyłączony (`PM_RAG_QUEUE_ENABLED`). Kolejka ma być ręczna: zapis do DB, właściciel przegląda i dopisuje do seedów.

### Zmiany
- `POST /api/dictionary/suggestions` — zawsze zapis `pending` w `dictionary_suggestions`.
- `/api/words/report` bez RAG przekierowany na ten sam zapis.
...
**Proces / intencja:** Domknięcie technicznego SEO bez strony partnerskiej. Search Console i DNS/www to kroki operatorskie (`deploy/SEO-ops.md`), nie kod.
...
**Proces / intencja:** Sprint SEO dodał statyczny blok „Jak grać?” pod listą pokoi; użytkownik zobaczył dwa razy to samo obok karuzeli. Decyzja: jedna warstwa treści w UI; SEO zostaje w meta, JSON-LD, sitemap i stronach prawnych — bez duplikatu wizualnego.
...
**Proces / intencja:** Lepsza synchronizacja stanu lobby. Nowy typ WebSocket `lobby_state` — pełny snapshot (ready_players, connected_players, host_name). Przygotowanie pod aplikację mobilną.

### Zmiany
- Backend: `lobby_state_payload()` + `_broadcast_lobby_state()` wysyłany przy ready/not_ready/join/disconnect/kick.
- Frontend: `onLobbyState` handler aktualizujący UI gotowości niezależnie od `score_update`.
- SonarCloud: fix redundant except w handlers.py, sync bez await w main.py, deduplikacja stringów w manager.py.
